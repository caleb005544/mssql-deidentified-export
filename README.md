...
graph TD
    %% 資料庫與來源
    subgraph "Data Source (On-Premise)"
        DB[(PostgreSQL / MSSQL)]
    end

    %% ETL 處理層
    subgraph "Python ETL Pipeline (Pandas & SQLAlchemy)"
        L1[<b>Layer 1: Ingestion</b><br/>Raw Data Extraction] --> L2[<b>Layer 2: Staging</b><br/>Cleaning & Typing]
        L2 --> L3[<b>Layer 3: Semantic</b><br/>Dimensions & Joins]
        L3 --> L4[<b>Layer 4: Measurement</b><br/>KPIs & RLS Tagging]
    end

    %% 自動化與派發
    subgraph "Automation & Delivery"
        Cron[[Unix Cron Job]] -- Triggers --> L1
        L4 --> L5{<b>Layer 5: Dispatch</b><br/>RLS Filtering}
        Map[/report_dispatch_map.csv/] -.-> L5
        L5 --> Excel[Excel Customization<br/>Openpyxl]
        Excel --> SMTP[SMTP Dispatch<br/>Authenticated SMTP]
    end
    ...
    style L5 fill:#fff4dd,stroke:#d4a017,stroke-width:2px
